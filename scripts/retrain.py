...
用 `task_train` 训练覆盖未来最多 20 个交易日的冻结候选模型段。每日入口检查目标日
是否已被 active artifact 覆盖，仅在新段首日训练。每个任务训练两个同特征模型；仅前一验证段的
...
def _gate_metrics(score: pd.Series, label: pd.Series,
    ...
    result.update(backtest_td0(score.to_frame("score"), start, end))
    return result
...
def _write_candidate_artifacts(recorder, gate: dict, task: dict,
                               output_root: Path | None = None) -> tuple[Path, Path]:
    """Write governance sidecars; model objects remain in Qlib Recorder."""
    from qlib_ifind_beta.live.intraday import write_json
    root = Path(output_root or Path(__file__).resolve().parent.parent / "data" / "model_governance")
    ...
    candidate = {
        ...
        "usable_from": segments["test"][0],
        "hflgb_experiment": ROLLING_EXPERIMENT, "hflgb_recorder_id": rid,
        "hflgb_artifact": "params.pkl", "xgb_experiment": ROLLING_XGB_EXPERIMENT,
        "xgb_recorder_id": gate.get("xgb_recorder_id", ""), "xgb_artifact": "params.pkl",
        "gate_artifact": ROLLING_GATE_ARTIFACT, "segments": segments,
        "gate_passed": bool(gate.get("gate_passed")),
        "ensemble_weight": float(gate.get("weight", 0.0)),
    }
    report = {
        "schema_version": 1, "candidate_recorder_id": rid,
        "validation_segment": segments["valid"], "test_segment": segments["test"],
        "validation_baseline": gate.get("validation_baseline", {}),
        "validation_candidate": gate.get("validation_candidate", {}),
        "all_gates_passed": bool(gate.get("gate_passed")),
        "promotion_recommendation": "REVIEW",
    }
    return (write_json(root / f"candidate-{rid}.json", candidate),
            write_json(root / f"validation-{rid}.json", report))


def promote_candidate(manifest_path: str | Path, approved_by: str,
                      output_path: str | Path | None = None) -> dict:
    """Explicitly promote one reviewed Candidate and update Qlib's online tag."""
    from qlib_ifind_beta.live.intraday import write_json
    manifest = json.loads(Path(manifest_path).read_text())
    if manifest.get("status") != "CANDIDATE":
        raise ValueError("only a CANDIDATE manifest can be promoted")
    ...
    recorder = R.get_recorder(recorder_id=manifest["hflgb_recorder_id"],
    ...
    active.update(status="ACTIVE", approved_by=approved_by,
                  approved_at=datetime.now().astimezone().isoformat(timespec="seconds"))
    target = Path(output_path or Path(__file__).resolve().parent.parent /
                  "data" / "model_governance" / "active_model_manifest.json")
    write_json(target, active)
    return active
...
def run(test_start: str | None = None,
    ...
    """Train a Candidate frozen block only when the target is not covered.
    ...
    # 3. 训练模型对并生成审阅材料；不会修改 Qlib online tag。
    new_ids, manifests, reports = [], [], []
    for i, task in enumerate(tasks):
        ...
        rid = _recorder_id(recorder)
        new_ids.append(rid)
        manifest, report = _write_candidate_artifacts(recorder, gate, task, output_root)
        manifests.append(str(manifest)); reports.append(str(report))
        print(f"         → recorder_id={rid} (CANDIDATE), gate_weight={gate['weight']:.2f}, "
              f"xgb_recorder_id={gate['xgb_recorder_id'] or 'FAILED'}")

    print(f"✅ retrain 完成：{len(new_ids)} 个模型对完成，状态=CANDIDATE")
    return {"n_new_tasks": len(new_ids), "new_recorder_ids": new_ids,
            "experiment": ROLLING_EXPERIMENT, "candidate_manifests": manifests,
            "validation_reports": reports, "published": False}
...
if __name__ == "__main__":
    ...
    p.add_argument("--promote-manifest", default=None)
    p.add_argument("--approved-by", default="")
    a = p.parse_args()
    result = (promote_candidate(a.promote_manifest, a.approved_by)
              if a.promote_manifest else run(a.test_start))
