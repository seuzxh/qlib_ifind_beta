...
| 09:41 第 11 根价量因子（8 候选 + 延迟执行对照） | 唯一双窗+PortAna 领先的 e94rng（振幅）在延迟执行下 W2 蒸发、purged 胜率 32% 淘汰；沉淀"延迟执行对照"方法论与 buy942o 研究资产 | [09:41 因子](backtest-log/2026-09-12-0941-bar-factors.md) |
| label 改用 close1500（15:00 分钟收盘，两腿同源） | 验证未通过（W2 崩塌 + purged 胜率 21%/37%）；混合基准"噪声"实为可学习结构，回测结算语义下旧 label 是忠实定义。**2026-09-12 用户决策采纳纯分钟版为后续训练基线**（生产评分不受影响） | [close1500 label](backtest-log/2026-09-12-label-close1500.md) |
