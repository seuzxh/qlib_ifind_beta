...
> **历史方案，已被 §60 取代。** 原 step=1、2 年/60 日窗口未通过后续严格 OOS 审计；
> 当前生产实现为 90/20/embargo1/test20 双模型门控，且 `use_online=True`。
...
## §51 策略层/组合层反拥挤优化（2026-07-12）

> 触发：联网搜索发现 2025 年量价因子普遍回撤、博弈类因子表现突出，策略拥挤与共振风险
> 是核心痛点。因子层已 10 重墙穷尽（§23-§31, §38, §39, §44），§25 T-1 尾盘因子已证伪，
> §34 反转 gate 已证伪。本节转向**策略层/组合层 overlay**——不改因子/模型/label，
> 纯在 topk 选股后叠加仓位/选股/加权调整。
> 脚本：[scripts/compare_anticrowd.py](../../scripts/compare_anticrowd.py)。复用
> `data/rolling_90d_result.pkl`（HFLGBModel 90 天滚动重训 361 天 OOS pred/label），无需重训。

### 对比表

| 方案 | 超额(net) | IR | 最大回撤 | Calmar | vs baseline |
|---|---|---|---|---|---|
| baseline (topk10 满仓) | 166.7% | 1.71 | -34.2% | 4.88 | — |
| G7 波动率目标仓位 | 129.5% | 1.60 | -31.7% | 4.09 | ↓ |
| CS 横截面分散度择时 | STOP | — | — | — | cheap-falsify 证伪 |
| CONCEPT 概念分散 | STOP | — | — | — | cheap-falsify 证伪 |
| 4a 动态 topk | 108.3% | 1.15 | -41.6% | 2.60 | ↓ |
| 4b score rank-weight | 125.1% | 1.36 | -38.5% | 3.25 | ↓ |

**结论：全部方案均不如 baseline，baseline topk10 满仓仍是最优。**

### 方案 1: G7 波动率目标仓位

position = median_vol / current_vol（10 日 trailing abs return vol，full-sample median）。
在 HFLGBModel pred 上 Calmar 4.09 < baseline 4.88。

**§48 复现失败的原因**：§48 的 G7 Calmar 6.20 是在 LGBModel pred 上得到的（§46 baseline）。
HFLGBModel pred 分布不同（binary loss 横截面 alpha），其收益曲线波动结构与 LGBModel 不同，
vol-target 减仓在 HFLGBModel 上反而削减了正常期收益（超额 166.7%→129.5%，-37pp），
而回撤改善有限（-34.2%→-31.7%，仅 -2.5pp）。§48 的回撤控制结论仅对 LGBModel pred 成立。

### 方案 2: CS 横截面分散度择时 — cheap-falsify STOP

- 分散度 = 每日池内 label cross-sectional std / |mean|（中位 4.37）
- Spearman corr(disp[T], top10_ret[T+1]) = **0.039**（p=0.464，不显著）
- 低分散日次日收益 0.41% vs 高分散日 0.68%，Mann-Whitney p = 0.115（>0.1）
- **判定 STOP**：横截面分散度无次日收益预测力，择时无米之炊

### 方案 3: CONCEPT 概念分散反拥挤 — cheap-falsify STOP

- 390 个同花顺概念板块，5449 票有映射
- top10 最大概念占比：median=0.40（即中位情况下 top10 里 40% 属同一概念）
- 高集中日收益 **0.83%** vs 低集中日 **0.20%**，Mann-Whitney p = 0.965
- **方向反转**：高概念集中日收益反而更高，概念集中 ≠ 回撤风险
- **判定 STOP**：概念集中度无负向收益预测力，概念上限反而会删掉收益最高的交易日

### 方案 4a: 动态 topk — 不如 baseline

- CV = std(pred)/|mean(pred)| per day。CV > 1.5 → topk=5；CV < 0.8 → topk=15；else 10
- Calmar 2.60 << baseline 4.88（回撤反而扩大 -41.6%）
- 原因：缩减 topk 降低分散度 → 个股冲击放大 → 回撤更大

### 方案 4b: score rank-weight — 不如 baseline

- topk10 内 rank 线性加权（rank 0 = 权重 10, rank 9 = 权重 1）
- Calmar 3.25 < baseline 4.88
- 原因：rank-weight 集中于 rank 0-2 的重仓票 → 个股 idiosyncratic 风险放大 → 回撤更大

### 第一性原理总结

1. **baseline topk10 equal-weight 满仓是结构性最优**：在 HFLGBModel pred 上，等权 top10
   的分散效果 + 满仓的收益效率已达到 Calmar 天花板。
2. **任何仓位调整（G7/CS）的代价 > 收益**：减仓削减正常期收益的幅度远大于降低回撤的幅度。
3. **任何集中化（4a/4b）的代价 > 收益**：个股 idiosyncratic 风险放大回撤。
4. **概念集中 ≠ 共振风险**：概念集中日的收益反而更高（0.83% vs 0.20%）——同概念票的
   集体上涨是 alpha 来源，不是风险来源。
5. **§48 G7 结论的模型依赖性**：vol-target 在 LGBModel 上有效（Calmar→6.20），但在
   HFLGBModel 上无效（Calmar→4.09）。HFLGBModel 的 binary loss + 横截面 alpha 口径
   使得收益曲线更平稳，vol-target 减仓反而过度。

**落档**：§51 反拥挤优化完成。所有 5 方案均不如 baseline。FROZEN champion 不变
（enhanced(18)@topk10/nd8 + HFLGBModel + 满仓 equal-weight）。策略层/组合层杠杆
与因子层一样触及天花板——baseline 已是最优形态。剩余改善方向只能在数据源扩展
（Level-2 / 集合竞价 / 另类数据）或解冻 label horizon（多日持有重设计）。

---

## §52 T-1 反转因子 + T-1 尾盘量比 cheap-falsify（2026-07-12）

> 触发：用户指示"试试 T-1 尾盘量比因子和反转因子"。
> **前提确认（避免重做）**：§25 测过 T-1 尾盘因子（tail_vol_ratio_t1）但那是
> LGBModel+topk20 口径（已证伪 IC −16%）；§34 测过 1d day_return 作为 gate proxy
> （IC −0.0429）。**多日（3d/5d/10d）反转因子从未作为模型特征被测过**。
> 当前条件变了：HFLGBModel + topk10/nd8 + 240 slot。按 CLAUDE.md 流程先 cheap-falsify。
> 脚本：[scripts/cheap_falsify_reversal_tail.py](../../scripts/cheap_falsify_reversal_tail.py)。

### Part 1: 反转因子 rank IC vs label

| horizon | raw_mom_IC | reversal_IC | ICIR | IC>0% |
|---|---|---|---|---|
| 1d | +0.3591 | −0.3591 | 2.80 | 99.7% |
| 3d | +0.2077 | −0.2077 | 1.36 | 90.6% |
| 5d | +0.1382 | −0.1382 | 0.83 | 79.8% |
| 10d | +0.1004 | −0.1004 | 0.61 | 74.0% |
| 20d | +0.0755 | −0.0755 | 0.44 | 67.6% |

**核心发现：883926 高 beta 池是动量延续型，不是反转型。**

raw momentum（close/ref(close,N)-1）的 IC 全线为正（1d=0.36, 5d=0.14），
意味着**过去 N 天涨的票，T+1 继续涨**——这是强动量效应。
反转因子 reversal_Nd = -momentum 的 IC 因此全线为负，方向完全错误。

> 反直觉但第一性原理自洽：883926 是同花顺高 beta 指数，编制逻辑就是选弹性最大的票。
> 高弹性 = 动量放大器 → 动量延续是其结构本质。反转因子与池性相斥。

### Part 2: T-1 尾盘量比 rank IC vs label

tail_vol_ratio_t1 = T-1 日尾盘 10min（14:51-15:00）成交量 / 前段 10min（09:31-09:40）成交量
...
**判定 STOP**：IC = 0.009（噪声级），ICIR = 0.02。T-1 尾盘量比对 T+1 收益无预测力。

与 §34 结论一致：盲区 A（T-1 minute 盘中形态）0/7 过线。§25 的 tail_vol_ratio_t1
在 LGBModel+topk20 上 |RankICIR|=0.398（#3 of 23），但作为单变量 IC 弱
（§34 已证 minute 盘中 IC 全 <0.015）。本次在 HFLGBModel 口径下单变量 IC = 0.009，
再次确认 T-1 尾盘信号无效。

### Part 3: mom_5d 与 champion 18 因子正交性

虽然反转方向错误，但 raw momentum_5d IC=+0.14 是很强的信号（远超 champion 18 里
任何单因子，overnight_gap +0.062 最强）。如果它与 champion 18 因子正交，
加 momentum_5d 可能有增量。

| 因子 | mean\|r\| | max\|r\| |
|---|---|---|
| **vol_vs_yest_t5** | 0.3274 | **0.6489** |
| overnight_gap | 0.2538 | 0.5360 |
| vol_vs_yest | 0.2534 | 0.4796 |
| startup_total | 0.2297 | 0.5024 |
| vol_vs_yest_t3 | 0.1715 | 0.5025 |
| accel_1m | 0.1385 | 0.3651 |
| accel_3m | 0.1384 | 0.3883 |
| accel_5m | 0.1400 | 0.3602 |
| startup_mom_5m | 0.1420 | 0.3582 |
| ... | ... | ... |

**mom_5d 与 vol_vs_yest_t5 高度共线**（max |r|=0.65）——两者都衡量"近几日量价趋势"。
vol_vs_yest_t5 是 T 日早盘量 / T-5 日全天量，隐含了 5 日量能动量。
mom_5d 是 5 日价格动量，两者通过"量价齐升"效应高相关。

**结论：mom_5d 的信息已被 champion 18 因子中的 vol_vs_yest 族吸收。**
IC=0.14 虽强，但 65% 被 vol_vs_yest_t5 解释。增量信息有限。

### 综合判定

| 方向 | IC | bar | 判定 |
|---|---|---|---|
| reversal_5d（反转） | −0.138 | 需正 IC | ✗ 方向错（池是动量延续型） |
| tail_vol_ratio_t1（T-1 尾盘量比） | 0.009 | >0.03 | ✗ 噪声级 |
| momentum_5d（动量，正方向） | +0.138 | IC 强 | △ 但与 vol_vs_yest_t5 共线 0.65 |

**全部 STOP，不推进物化。** 原因：

1. **反转因子方向错误**：高 beta 池 = 动量延续型，反转 IC 全线为负。加反转因子等于
   逆向交易池性，必然毁收益。
2. **T-1 尾盘量比无信号**：IC=0.009（噪声级），与 §34 盲区 A 结论一致。
3. **raw momentum 虽 IC 强但已被吸收**：mom_5d IC=0.14 与 vol_vs_yest_t5（IC −0.069）
   共线 0.65，信息已被 champion 覆盖。加它会导致 §25 式的"add-factor re-ranking"
   稀释效应（IC↑但头部超额↓）。

**落档**：§52 反转+尾盘 cheap-falsify 完成。三个方向全部 STOP。
FROZEN champion 不变（enhanced(18)@topk10/nd8 + HFLGBModel）。
新发现：883926 高 beta 池 = 动量延续型（1d mom IC=0.36），非反转型。
因子层 + 策略层 + 组合层三重穷尽后，champion 已达结构性天花板。

---

## §53 回撤根因深度诊断：追高拥挤假说 vs 数据（2026-07-12）

> 触发：用户提出"最大回撤很可能是由拥挤度过高导致的追高造成，能否找到因子避免？"。
> 本节用数据回答：**top10 选出的票是否在追高？动量极端高位是否预测次日崩溃？**
> 脚本：[scripts/diag_drawdown_chasing.py](../../scripts/diag_drawdown_chasing.py)。

### 回撤定位

- **最大回撤：-34.16%**
- **峰值→谷值：2025-02-24（cum=1.194）→ 2025-04-07（cum=0.786），42 天**
- 回撤期 30 个交易日，日均净收益 -1.28%

### 诊断 1：回撤期 top10 并非追高（追高假说推翻）

| 指标 | 回撤期 (DD<-5%) | 正常期 (DD>-2%) | Δ |
|---|---|---|---|
| top10 1d mom | +0.84% | +1.59% | -0.75pp |
| top10 3d mom | +12.94% | +14.50% | -1.56pp |
| top10 5d mom | **+14.88%** | **+17.13%** | **-2.25pp** |
| top10 10d mom | +18.86% | +22.82% | -3.96pp |
| top10 vol_ratio | +1.438% | +1.484% | -0.046pp |

**Mann-Whitney U（DD 期 mom > 正常期）p=0.9996 → 完全相反方向。**

回撤期的 top10 近期涨幅**更低**（5d mom 14.88% vs 17.13%），不是追高。
追高假说被数据彻底推翻。

### 诊断 2：动量五分位 → 次日收益（完全单调，无追高崩溃）

| 分位 | 均值 Label | 描述 |
|---|---|---|
| Q1 (最低动量) | **-1.20%** | 低动量 → 次日跌 |
| Q2 | -0.58% | |
| Q3 | -0.21% | |
| Q4 | +0.27% | |
| Q5 (最高动量) | **+1.88%** | 高动量 → 次日继续涨 |

**Q5 > Q4 > Q3 > Q2 > Q1，完全单调递增。极端高动量非但不崩溃，反而次日收益最高。**
动量在全部范围内都是正向的，不存在非线性阈值。

### 诊断 3：top10 内部高动量子集收益更高（追高即正确）

将每日 top10 按自身 5d mom 分成高 5 和低 5：

- **高动量 top5 日均收益：+1.41%**
- **低动量 top5 日均收益：-0.38%**
- 差值 **+1.79%/day**（Mann-Whitney p=1.0，高动量子集显著更优）

在 top10 内部，**近期涨得更多的票次日继续涨得更多**。追高不但不会崩溃，
反而是 alpha 的来源。

### 诊断 4：动量阈值条件收益（无安全边际可言）

| 阈值 | 高于天数 | 高于均值收益 | 低于天数 | 低于均值收益 | p-value |
|---|---|---|---|---|---|
| 2% | 359 | +0.28% | 2 | +6.68% | — |
| 5% | 350 | +0.34% | 11 | -0.46% | 0.59 |
| 10% | 306 | +0.46% | 55 | -0.50% | 0.95 |
| 15% | 183 | +0.75% | 178 | -0.13% | 1.00 |
| 20% | 68 | **+0.99%** | 293 | +0.16% | 0.97 |

**所有阈值下：高于阈值的天数收益 ≥ 低于阈值的天数。**
top10 均值 5d mom > 20% 时收益最高（+0.99%/day）。
没有任何阈值能让"高动量 → 低收益"。**无法通过动量过滤来避免回撤。**

### 诊断 5：回撤期逐日明细

回撤期前 5 天（2025-02-24→02-28）的关键特征：

```
Date         net_ret   mom5d    mom1d     DD
2025-02-24    +0.82%   13.35%   -0.36%    0.0%   ← peak
2025-02-25    -0.68%   14.90%   +0.46%   -0.7%
2025-02-26    +0.68%   18.28%   +2.40%   -0.0%
2025-02-27    -4.80%   21.93%   +2.56%   -4.8%   ← 单日暴跌
2025-02-28    -3.86%   16.74%   -2.73%   -8.5%   ← 崩溃延续
```

2025-02-27 单日 -4.8% 是回撤的加速点。当日 top10 的 5d mom=21.93%（偏高），
1d mom=2.56%（偏高），但**这与高 beta 池在市场调整期的 beta 放大效应一致**，
不是个股层面的追高。

回撤期全段：top10 均值 5d mom=12.64%（低于正常期 17.13%），
**策略不是在追高，而是被高 beta 池在市场调整期的系统性下跌拖累**。

### 综合结论：回撤根因 = 系统性 beta 风险，非追高拥挤

1. **追高假说推翻**：回撤期 top10 近期涨幅**低于**正常期，不是追高。
2. **动量完全单调**：Q5（最高动量）次日收益 +1.88%，无任何崩溃迹象。
3. **追高即正确**：top10 内部高动量子集收益 +1.79%/day 高于低动量子集。
4. **无阈值效应**：所有动量阈值下，高于阈值的日期收益 ≥ 低于阈值。
5. **回撤 = 系统性**：2025-Q1 市场调整期（benchmark 也跌），高 beta 池 beta≈1.03
   被动放大基准跌幅，是个股无法规避的系统性风险。

**落档**：§53 追高拥挤假说被数据彻底推翻。回撤根因是系统性 beta 风险
（§47/§48 已确认），非策略追高。不存在"动量过滤因子"能避免此类回撤——
因为高动量恰恰是收益来源，过滤高动量等于删掉 alpha。
FROZEN champion 不变。回撤控制唯一有效的方案仍是 §48 G7 波动率仓位
（在 LGBModel 口径下 Calmar→6.20），但在 HFLGBModel 口径下无效（§51 已测）。

---

## §54 回撤期赢家画像：谁在跌市中逆势上涨？（2026-07-12）

> 触发：用户提问"2025-02-24→04-07 A 股调整期，高 beta 池中有没有收益很好的个股？它们的关联性如何？"
> 脚本：[scripts/diag_drawdown_winners.py](../../scripts/diag_drawdown_winners.py)（区间口径，**已废弃**）
> + [scripts/diag_drawdown_winners_daily.py](../../scripts/diag_drawdown_winners_daily.py)（逐日时变池口径，**正确**）。

> **⚠️ 口径修正（用户指出）**：初版用 361 天池并集（4661 只）做区间收益分析，
> 违反策略前提——883926 是**时变日频池**（每日 ~90 只，日换手 89%）。
> 策略每日换仓持有 1 天（label = T 日 9:41 → T+1 收盘），正确口径是**逐日分析日内池**。
> 以下为修正后的逐日时变池口径结果，初版区间口径结论作废。

### 修正后核心对比：回撤期 vs 正常期（逐日时变池）

| 指标 | 回撤期 (30天) | 正常期 (39天) |
|---|---|---|
| 日均池大小 | 89.2 只 | 89.2 只 |
| 日内赢家率（label>0） | **39.1%** | 45.5% |
| 池均值 label | **-1.07%** | +0.22% |
| top10 均值 label | **-1.09%** | +1.26% |
| **top10 - pool alpha** | **-0.019%** | **+1.04%** |

### 核心发现 1：回撤期策略 alpha 几乎消失

正常期 top10 相对池均值有 +1.04%/day 的 alpha，而**回撤期 alpha 归零**（-0.019%）。
策略选出的 top10 与池均值几乎无差异——选股信号在回撤期**完全失效**。

这是回撤的微观根因：不是 top10 选了更差的票（top10 = -1.09% ≈ pool = -1.07%），
而是**策略无法从高 beta 池中区分出赢家**——整个池同步下跌，选股 alpha 失效。

### 核心发现 2：策略 top10 命中率验证 alpha 消失

| 指标 | 回撤期 | 正常期 | 随机基准 |
|---|---|---|---|
| top10 在日内 label 前 25% 的比例 | **26.0%** | 31.0% | 25% |
| 富集倍数 | **1.04x** | 1.24x | 1.0x |

回撤期富集仅 1.04x（几乎随机），正常期 1.24x。

### 核心发现 3：回撤期日内赢家 = 高动量（与 §52 一致，与初版相反）

| 特征 | 赢家 (label>0) | 输家 (label≤0) | p-value | 方向 |
|---|---|---|---|---|
| 1d 动量 | +1.59% | -1.75% | **0.0000** | W > L |
| 5d 动量 | +15.4% | +12.4% | **0.0000** | W > L |
| 10d 动量 | +19.7% | +16.0% | **0.0000** | W > L |
| pred score | +0.009 | -0.001 | 0.052 | W > L |
| 价格 | 101.4 元 | 69.6 元 | 0.18 | — |

**日内赢家是高动量的票**（5d mom +15.4% vs +12.4%，p<0.001）——
与 §52 结论完全一致（池是动量延续型）。
初版区间口径的"赢家 = 低动量低价"结论是**口径错误导致的虚假发现**。

### 核心发现 4：回撤期 1d 动量 IC 极高但策略未能利用

| 特征 | 横截面 rank IC vs label | IC>0 率 |
|---|---|---|
| **1d 动量** | **+0.384** | **100%** |
| pred score | +0.079 | 73.3% |
| 5d 动量 | +0.077 | 63.3% |
| 10d 动量 | +0.060 | 63.3% |

回撤期日内 1d 动量的 IC = 0.384（30 天全 > 0），**信号极强**——
昨日涨的票今日继续涨。但策略的 pred score IC 仅 0.079——
说明策略未能充分捕捉这个信号。

### 第一性原理修正总结

1. **回撤微观根因 = 选股 alpha 消失**：不是策略追高，而是策略在高 beta 池全跌时
   失去了区分能力（top10 ≈ pool，富集 1.04x）。
2. **日内赢家仍是高动量**（1d mom IC=0.384）——与池性一致（§52 动量延续型），
   初版"赢家 = 低价低动量"是口径错误的虚假发现。
3. **策略信号在回撤期衰减**：pred IC 0.079 vs 1d 动量 IC 0.384，
   说明 18 个分钟因子未能有效提取 1d 动量信号——但加 1d 动量因子会与 vol_vs_yest_t5
   共线 0.65（§52），且在正常期已被吸收。
4. **池的系统性 beta 风险不可通过选股规避**：回撤期整个池 39% 赢 / 61% 输，
   策略的 alpha（1.04x）不够覆盖 beta 裸露。

**落档**：§54 修正为逐日时变池口径。核心发现：回撤 = 策略选股 alpha 消失
（top10 ≈ pool），而非追高。日内赢家仍是高动量票（与池性一致）。
FROZEN champion 不变。初版区间口径脚本保留为反面教材。
...
> 本节用新思路改进仓位层。脚本：`scripts/compare_position_sizing.py`（worktree `feat/position-sizing`）。
...
### C1 参数鲁棒性（threshold 0.5%~3% → Calmar 7.6~10.0，宽峰非尖锐）

### 最优方案选择：C1（基准趋势连续仓位）

**Calmar 4.88→9.56（+96%），DD -34%→-18.5%（-15.7pp），excess 不降反升（167%→177%）。**

选择理由：简单直观（单一基准动量信号）+ 子时段稳定 + 参数鲁棒 + 无前视。
...
| 文件 | 改动 |
|---|---|
| `position_sizing.py` | 新增：`compute_benchmark_position()` + `compute_position_for_day()` 纯函数 |
| `config.py` | 新增：`POSITION_WINDOW=20`, `POSITION_THRESHOLD=0.02`, `POSITION_FLOOR=0.3` |
| `live/track.py` | `compute_nav()` 加 `position_scale` 参数（默认 None = 向后兼容） |
| `test_position_sizing.py` | 新增 15 测试（边界/无前视/向后兼容/仓位缩放） |

**设计决策**：不改 `td0_strategy.py`（保持 shift=0 单行差异守卫）；仓位 overlay 在 NAV 计算层
（`compute_nav`）应用，不影响选股逻辑；`position_scale=None` 时向后兼容。

### B3 (IC z-score) 前视偏差证伪（2026-07-13）
...
仓位层作为 overlay 在 NAV 计算层应用。代码在 `feat/position-sizing` worktree。

### §55 勘误：C1 同样存在当日基准收益前视（2026-07-16）

上面的“C1 不受影响”结论错误。批量函数原实现先计算包含 T 日收盘收益的 20 日
benchmark momentum，再将结果直接应用到 T 日 09:41 的仓位；但该时点尚不知道 T 日
收盘，因此仍是前视。单日实时函数本来就排除了 target_date，批量与实时口径不一致。

修复方式：在 rolling momentum 前对 benchmark return `shift(1)`，并新增“改变 T 日
收益不得改变 T 日仓位”的回归测试。按相同 361 日预测快照重算快速代理：

| 方案 | 绝对累计 | 几何累计超额 vs SH300 | 标准年化 Calmar | 最大回撤 |
|---|---:|---:|---:|---:|
| 满仓 baseline | +166.7% | +118.1% | 2.85 | -34.2% |
| C1（修复后） | +87.6% | +53.4% | 2.43 | 约 -25% |

同时，旧脚本把绝对策略收益称为 `excess`，并用累计收益除以回撤称为 Calmar；现已分别
改为几何超额、active information ratio 和“年化收益 / 最大回撤”的标准定义。

**最终裁决：撤销 C1 晋级与 Calmar 9.56 结论。当前默认满仓；仓位模块仅保留为研究接口。**

### §56 真实换手率确认因子：方向成立，但暂不晋级（2026-07-16）

在冠军 Top20 内构造盘前可知的真实换手率确认项：

`gap_x_open_turn = overnight_gap × log1p(vol_vs_yest × turn[T-1] / 240)`

其中 `turn[T-1]` 来自 iFinD 日频换手率，T 日 09:41 前已知；`vol_vs_yest` 只使用
T 日 09:31–09:40 成交量和 T-1 全天分钟量。已回补 5,118 只股票、2024-01-01 至
2026-07-02 共 3,086,154 行，并物化 5,042 只（其余 76 只与分钟数据缺失名单一致）。

冻结冠军 Top20 内的条件 IC 三个窗口同向为正：2025Q2 `+0.1250`、2026Q2
`+0.0439`、独立留出 2025Q4 `+0.0560`。这是本轮首个跨窗口方向稳定的新数据因子。

但三种晋级路径均未通过“IC 与精确 TD0 超额同时改善”的门槛：

| 路径 | 2025Q2 | 2026Q2 | 独立留出 2025Q4 | 裁决 |
|---|---|---|---|---|
| Top20 固定重排 | `w=.75` 超额 32.5%→53.3% | 148.2%→190.1% | 75.1%→40.4% | 留出退化，拒绝 |
| 19 因子 HFLGB 入模 | IC 0.0610→0.0606，超额→-0.7% | IC 0.0548→0.0503，超额→165.9% | IC 0.0593→0.0574，超额→66.6% | 分裂预算不稳定，拒绝 |
| 全截面 rank 小权重融合 | 2025Q2 可改善 | 2026Q2 超额均下降 | 2025Q4 IC/RankIC 下降 | 无统一权重，拒绝 |

独立留出窗口是在因子公式和权重网格冻结后才训练，避免把第三个季度继续当作选参样本。
同时修复了早期重排脚本把预测面板截成 Top20、导致广义 IC 与持仓继承失真的验证错误；
最终数字均保留完整预测面板并调用真实 `TopkDropoutStrategyTD0(topk=10,n_drop=8)`。

**最终裁决：冠军仍为 18 因子 HFLGB；真实换手率因子保留为研究候选，不进入实时链路。**
下一阶段只考虑两条可证伪路线：（1）用严格 walk-forward 的训练期数据决定动态重排权重；
（2）使用直接针对 Top10 尾部的排序/分组目标，避免二分类树把强条件信号用于错误分裂。

### §57 尾部目标与回归集成：同源特征重组无法稳定提升（2026-07-16）

按 §56 的路线继续验证三类目标函数，全部保持 18 因子、label、训练区间和精确 TD0
策略不变：

1. `LambdaRank + NDCG@10`：验证 NDCG 噪声过大，2025Q2 第 2 轮即早停，IC 0.0196、
   超额 -47.3%，直接证伪。
2. 每日 Top20 二分类专家：2025Q2 单模型超额 133.9%，但 2025Q4 RankIC 为 -0.0220；
   固定 z-score 融合和验证期动态选权均未跨窗口通过。
3. 连续收益 LGB 回归：361 日滚动 OOS 预筛中，二分类/回归 50:50 将 IC
   0.0559→0.0665、RankIC 0.0676→0.0734、Top10 日均收益 0.530%→0.634%，但精确
   TopkDropout 持仓路径仍呈明显非线性。

回归融合的长窗口精确结果：

| 窗口 | 验证期选权 | 测试 IC | 测试 RankIC | 测试年化超额 | 相对冠军 |
|---|---:|---:|---:|---:|---|
| 2025Q2 | 0% | 0.0610 | 0.1022 | 32.5% | 保持冠军 |
| 2026Q2 | 50% | 0.0580 | 0.0668 | 149.0% | 三指标小幅改善 |
| 2025Q4 | 25% | 0.0691 | 0.0830 | 81.2% | 三指标改善 |
| **独立留出 2024Q4** | **100%** | **0.0452** | **0.0553** | **-74.3%** | 冠军超额 +0.8%，严重退化 |

2024Q4 在公式、候选权重和选权规则冻结后才加入；它证明验证期选择无法可靠识别模型的
尾部失效阶段。**最终裁决：LambdaRank、Top20 专家和二分类/回归动态集成全部不晋级。**

下一阶段停止重组同一 18 因子，转向 09:41 机制真正缺失的数据：09:15–09:25 集合竞价
撤单/未匹配量、09:25 成交量与委买委卖不平衡、开盘后逐笔主动买卖方向。新数据必须先
验证 T 时点可得性和历史覆盖，再进入相同四窗口联合门槛。

### §58 集合竞价成交量：条件信号成立，walk-forward 选权拒绝（2026-07-16）

iFinD `high_frequency` 接口确认可以返回 09:15–09:30 区间的 09:30 集合竞价汇总 bar；
现有分钟库从 09:31 开始，确实遗漏了这段信息。按冻结冠军在测试期和验证期的 Top20
候选集合回填 219,811 行、2,510 只股票，并与 T-1 日成交量及现有开盘因子对齐。因子在
09:40 已完全可知，无标签或收盘数据前视：

`gap_x_auction = overnight_gap × log1p(auction_volume / (volume[T-1] / 240))`

测试期冠军 Top20 条件 RankIC 在两个可访问窗口同向为正：2026Q2 `+0.0345`、2025Q4
`+0.0603`；Top10 分别为 `+0.0426`、`+0.0480`。单独竞价量则跨窗口反号，已直接淘汰。

固定重排存在事后看起来有效但不可统一的权重：

| 测试窗口 | 事后最小通过权重 | IC | RankIC | 年化超额 | 冠军年化超额 |
|---|---:|---:|---:|---:|---:|
| 2026Q2 | 0.50 | 0.0552 | 0.0617 | 159.8% | 148.2% |
| 2025Q4 | 0.10 | 0.0597 | 0.0770 | 77.2% | 75.1% |

由于不存在同时通过两窗的固定权重，进一步采用严格 walk-forward：仅在各自验证期从
`{0,.10,.25,.50,.75}` 中选择，先要求 IC 与 RankIC 不低于验证期冠军，再最大化精确 TD0
超额，最后冻结到测试期。两个窗口都选择 `0`：2026Q2 对应验证期中，`.75` 虽提升
IC/RankIC，但超额由 `+42.0%` 降至 `-18.1%`；2025Q4 对应验证期中，所有非零权重的
RankIC 均略低于基线。因 iFinD 试用账户仅允许最近一年历史，后者验证期只能覆盖
2025-07-16 至 2025-09-30，且 2025Q2/2024Q4 无法回补；这是本结论的已知覆盖限制。

**最终裁决：竞价交互项保留为有经济含义的研究候选，但不进入实时链路，冠军仍为 18
因子 HFLGB。** 若取得正式账户的长历史集合竞价/盘口数据，应先补齐四窗口后原样复跑
冻结规则；在此之前不按测试期权重部署。

### §59 09:25 历史盘口：接口成立，但试用历史仅一个月（2026-07-16）

iFinD 官方 `snap_shot` 接口和两只股票实测均确认可回放历史盘口，包括时间戳、买一/卖一
价格及队列量、买二/卖二价格及队列量。样例 `000001.SZ` 在 2026-06-30 09:25:00 返回
买一队列 40,100、卖一队列 7,400，证明可构造 09:41 前已知的未成交供需不平衡。

但试用账户对 snapshot 的历史权限只有最近一个月（接口错误码 `-4310`），无法覆盖 §58
的验证/测试季度。改为逐交易日只拉当天冠军 Top20 后，完成可访问的 2026-06-16 至
2026-07-02：12 日、240 个股票日、478 条 09:24:55–09:25:00 快照。探索性条件 RankIC：

| 因子 | RankIC | ICIR | 日胜率 |
|---|---:|---:|---:|
| 买一/卖一队列不平衡 | +0.0130 | +0.052 | 50.0% |
| 两档合计队列不平衡 | -0.0025 | -0.011 | 58.3% |
| 买一减卖一剩余量 / 竞价成交量 | +0.0290 | +0.119 | 58.3% |
| 两档剩余量 / 竞价成交量 | -0.0135 | -0.057 | 66.7% |

样本只有 12 日，不能用于选权或晋级；同时最佳 RankIC 也没有强到足以支持例外处理。
**最终裁决：不做盘口重排，不改冠军。** 已保留 snapshot 客户端、字段测试、断点回填和
诊断脚本；若升级正式账户，应补齐四窗口后再进入固定的 IC + 精确 TD0 联合门槛。

### §60 XGBoost 无泄漏滚动门控：联合晋级并完成部署实现（2026-07-16）

在同一 18 因子和 label 下，增加
`XGBModel(reg:squarederror)` 作为异构树模型。单独 XGBoost 在 2025Q2、2025Q4 的
IC/RankIC 都高于 HFLGB，但 2026Q2 与 2024Q4 并不稳定；季度验证选权和全期固定权重
也分别在 2024Q4 超额或完整持仓路径上失效，因此两者均不直接晋级。

初版 90 日训练 / 20 日验证 / 20 日测试 walk-forward 曾得到 IC `0.05593→0.05854`、
年化超额 `60.08%→67.75%`，但后续部署审计发现 label[T] 要到 T+1 收盘才完整，而验证
期与测试期相邻会让验证末日 label 使用测试首日收盘。该结果存在一日边界泄漏，已撤销。

最终切分在每个测试段前增加 **1 个交易日 embargo**：90 日训练、20 日验证、1 日隔离、
最多 20 日冻结测试。验证末日 label 只使用 embargo 日收盘，在测试首日 09:41 前已知；
模型早停、门控选择和精确回测均不再接触测试期数据。

最终冻结的规则只考虑 25% XGBoost 融合，并在每个滚动段的前一验证期同时满足以下三项
时启用，否则保持 100% HFLGB：

1. IC 不低于 HFLGB；
2. RankIC 不低于 HFLGB；
3. 精确 `TopkDropoutStrategyTD0(topk=10,n_drop=8)` 年化超额不低于 HFLGB。

对 2025-01-02 至 2026-07-02 的 19 个严格 walk-forward 测试段重新训练两套模型，门控
在 11 段启用。拼接 361 个 OOS 交易日后统一做精确回测：

| 方案 | IC | RankIC | 年化超额 | 超额 IR | 绝对最大回撤 | 超额最大回撤 |
|---|---:|---:|---:|---:|---:|---:|
| HFLGB 基线 | 0.05542 | 0.06953 | 36.69% | 1.019 | -32.12% | -27.74% |
| **验证期三门槛滚动门控** | **0.05933** | **0.07349** | **51.55%** | **1.472** | **-29.56%** | **-20.58%** |
| 固定 25% XGBoost | 0.06245 | 0.07541 | 51.09% | 1.448 | -33.20% | -25.43% |

门控方案同时提升 IC `+7.1%`、RankIC `+5.7%`、年化超额 `+14.86pct`、超额 IR
`+44.5%`，并改善两类最大回撤。固定混合的 IC 更高，但绝对回撤变差；门控在收益近似的
同时显著改善回撤，故仍选择门控。11 个启用段中 9 个下一测试段 IC/RankIC 同时为正；
361 日日度 IC 差的 20 日移动块 bootstrap（20,000 次）95% 区间为
`[+0.00159,+0.00704]`，增量为正的比例为 99.95%。

**最终裁决：该规则晋级为当前优化冠军。** `scripts/retrain.py` 已实现 90/20/embargo1/
test20 双模型训练、精确三门槛、冻结 artifact 后发布；盘后与 09:41 纯内存推理均优先消费
覆盖目标日的 online 模型。artifact 缺失/损坏、日期不匹配、XGBoost 训练或加载失败时，
先退回有效 rolling HFLGB，再退回冻结生产冠军。行情数据完整到 2026-07-14，但交易日历
尚未包含下一目标交易日；历史冒烟 recorder 未标记 online。日历刷新后须在目标交易日盘前
运行重训入口才会切换，不宣称已在真实账户成交。

无泄漏复现实验为 `scripts/validate_xgb_purged_rolling_gate.py`，结果保存在
`data/xgb_purged_rolling_gate_ab.json`；旧 `validate_xgb_rolling_gate.py` 仅保留为发现边界
泄漏的审计记录，不再作为晋级依据。
